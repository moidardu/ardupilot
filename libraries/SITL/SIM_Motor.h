...
public:
    ...
    // float twist_angle_factor_sin = 0.012; //twist factor in frame
    float twist_angle_factor_sin = 0.00; //twist factor in frame

    float p_out_compensation_dist = 0.0; 


    // float motor_angle_factor = 0.00; // ~ 0 degree motor angled in/out
    // 
    float motor_angle_factor = 0.035; // motor angle factor for uniform yaw bias, Y.Out is from exp. is 0.14, this correlates to 0.7 degrees
    float motor_angle_factor_motor_loss_variance = 0*3.55*motor_angle_factor; //i.e. motor 6 has 0.8 deg, but motor 4 has 0.6
...
// /*
    	if (_display_order ==1){ //motor 1
            ...
            thrust_vector.y = motor_angle_multiplier*motor_angle_factor; // H normal motor angle - fitting to exp results
